function [h_out,x_g_out,iter] = thickness_wGL_solve_fix(h,x_g,u,parameters,flags)
...
Fadv_plus(n_elements) = u_eff_GL.*(h_Nplus_ghost.*(1-theta_GL) + h(n_elements).*theta_GL);% +...
...
Fadv_minus(1) = -(u(1)./x_g).*h(1);

%Compute stretching term
Fstretch = dsigma.*h.*(1-(x_g_old./x_g))./dtau;

%Compute source term
Fsource = -accumulation(parameters).*dsigma;
...
dFdh_plus(2:n_elements) = ((u_int - sigma_int.*x_g_deriv)./(x_g)).*(1-theta_int);

%j-1 thickness term (on thickness)
dFdh_minus = zeros(n_elements,1); %ensure that first term is zero
dFdh_minus(1:n_elements-1) = -((u_int - sigma_int.*x_g_deriv)./(x_g)).*theta_int;

%j thickness term (on thickness)
dFdh_zero = zeros(n_elements,1);
dFdh_zero(2:n_elements-1) = (dsigma(2:n_elements-1)/dtau) + ((u_plus(2:n_elements-1) - sigma_plus(2:n_elements-1).*x_g_deriv)./(x_g)).*theta_plus(2:n_elements-1) -...
                            ((u_minus(2:n_elements-1) - sigma_minus(2:n_elements-1).*x_g_deriv)./(x_g)).*(1-theta_minus(2:n_elements-1)) +...
                           dsigma(2:n_elements-1).*(1-(x_g_old./x_g))./dtau;

dFdh_zero(1) = (dsigma(1)/dtau) + ((u_plus(1) - sigma_plus(1).*x_g_deriv)./(x_g)).*theta_plus(1) -...
                            ((u_minus(1))./(x_g)) + dsigma(1).*(1-(x_g_old./x_g))./dtau;

dFdh_zero(n_elements) = (dsigma(n_elements)/dtau) + ((u_plus(n_elements) - x_g_deriv)./(x_g)) -...
                            ((u_minus(n_elements) - sigma_minus(n_elements).*x_g_deriv)./(x_g)).*(1-theta_minus(n_elements)) +...
                            dsigma(n_elements).*(1-(x_g_old./x_g))./dtau;
...
dFdx_g(1:n_elements-1) = -(1/(x_g.^2)).*(u_int + (sigma_int.*x_g_old./dtau)).*...
    (h(ind_e_plus).*(1-theta_int) + h(ind_e_minus).*theta_int) +...
                          ((u_int - sigma_int.*x_g_deriv)./x_g).*...
    (h(ind_e_plus).*(-dthetadx_g_int) + h(ind_e_minus).*dthetadx_g_int);

dFdx_g(n_elements) = -(1/(x_g.^2)).*(u(n_nodes) + (sigma_node(n_nodes).*x_g_old./dtau)).*...
    (h_Nplus_ghost.*(1-theta_GL) + h(n_elements).*theta_GL) +...
                          ((u(n_nodes) - sigma_node(n_nodes).*x_g_deriv)./x_g).*...
    (h_Nplus_ghost.*(-dtheta_dx_g_GL) + h(n_elements).*dtheta_dx_g_GL);

%Fadv_minus term
dFdx_g(2:n_elements) = dFdx_g(2:n_elements) + (1/(x_g.^2)).*(u_int + (sigma_int.*x_g_old./dtau)).*...
    (h(ind_e_minus).*theta_int + h(ind_e_plus).*(1-theta_int)) -...
                          ((u_int - sigma_int.*x_g_deriv)./x_g).*...
    (h(ind_e_minus).*dthetadx_g_int + h(ind_e_plus).*(-dthetadx_g_int));

%Stretch and source terms
dFdx_g(1:n_elements) = dFdx_g(1:n_elements) + dsigma.*h.*(x_g_old./(x_g^2))./dtau;
